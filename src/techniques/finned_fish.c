...
static int find_base_sets(Grid *grid, int size, UnitType unit_type, int value,
                          UnitSubset out[]);
static int find_cover_sets(UnitSubset bases[], int size, UnitSubset out[9]);
...
static void find_actual_covers(UnitSubset covers[], int num_covers,
                               ...
                               int out[]);
static int find_removals(Grid *grid, int size, int value, int base_idxs[],
...
static bool finned_n_fish_unit(Grid *grid, Step *step, int size,
    ...
    for (int value = 1; value <= 9; value++) {
        UnitSubset bases[9];
        int num_bases = find_base_sets(grid, size, unit_type, value, bases);

        if (num_bases < size) continue;

        int num_base_combs;
        UnitSubset **base_combs = generate_combinations(
            bases, num_bases, size, sizeof(UnitSubset), &num_base_combs);
        ...
        for (int base_comb_i = 0; base_comb_i < num_base_combs; base_comb_i++) {
            ...
            int num_covers = find_cover_sets(base_comb, size, covers);
            if (num_covers <= size || num_covers > size + 2) continue;
            ...
            for (int fin_unit_comb_i = 0; fin_unit_comb_i < num_fin_unit_combs;
                ...
                s->num_removals = find_removals(
                    grid, size, value, s->base_idxs, s->cover_idxs,
...
static int find_cover_sets(UnitSubset bases[], int size, UnitSubset out[9]) {
    for (int base_i = 0; base_i < size; base_i++) {
...
static int find_removals(Grid *grid, int size, int value, int base_idxs[],
    ...
    for (int i = 0; i < 9; i++) {
        ...
        if (grid_cell_has_cand(grid, cell, value)
            && arr_contains(cover_idxs, size, cell_unit(cell, cover_unit_type))
            && !arr_contains(base_idxs, size,
