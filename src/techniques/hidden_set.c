...
bool hidden_set(Grid *grid, Step *out_step) {
    bool pair_result = hidden_pair(grid, out_step);
    if (pair_result) return true;

    bool triple_result = hidden_triple(grid, out_step);
    if (triple_result) return true;

    bool quad_result = hidden_quad(grid, out_step);
...
bool hidden_pair(Grid *grid, Step *out_step) {
    out_step->tech = TECH_HIDDEN_PAIR;
    out_step->as.hidden_set.size = 2;

    bool row_result = hidden_n_set_unit(grid->rows, out_step, 2, UNIT_ROW);
    if (row_result) return true;

    bool col_result = hidden_n_set_unit(grid->cols, out_step, 2, UNIT_COL);
    if (col_result) return true;

    bool box_result = hidden_n_set_unit(grid->boxes, out_step, 2, UNIT_BOX);
...
bool hidden_triple(Grid *grid, Step *out_step) {
    out_step->tech = TECH_HIDDEN_TRIPLE;
    out_step->as.hidden_set.size = 3;

    bool row_result = hidden_n_set_unit(grid->rows, out_step, 3, UNIT_ROW);
    if (row_result) return true;

    bool col_result = hidden_n_set_unit(grid->cols, out_step, 3, UNIT_COL);
    if (col_result) return true;

    bool box_result = hidden_n_set_unit(grid->boxes, out_step, 3, UNIT_BOX);
    if (box_result) return true;
...
bool hidden_quad(Grid *grid, Step *out_step) {
    out_step->tech = TECH_HIDDEN_QUAD;
    out_step->as.hidden_set.size = 4;

    bool row_result = hidden_n_set_unit(grid->rows, out_step, 4, UNIT_ROW);
    if (row_result) return true;

    bool col_result = hidden_n_set_unit(grid->cols, out_step, 4, UNIT_COL);
    if (col_result) return true;

    bool box_result = hidden_n_set_unit(grid->boxes, out_step, 4, UNIT_BOX);
    if (box_result) return true;
...
static bool hidden_n_set_unit(Cell *units[9][9], Step *out_step, int set_size,
    ...
    for (int unit_i = 0; unit_i < 9; unit_i++) {
        ...
        if (num_missing_values < set_size) continue;

        int num_combs;
        int **combs = generate_combinations(missing_values, num_missing_values,
                                            set_size, sizeof(int), &num_combs);

        for (int comb_i = 0; comb_i < num_combs; comb_i++) {
            CandSet comb_set = cand_set_from_arr(combs[comb_i], set_size);
